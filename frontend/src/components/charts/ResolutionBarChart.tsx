...
export const ResolutionBarChart: React.FC<ResolutionBarChartProps> = ({ data }) => {
  ...
  const chartData = data
    .map(item => ({
      ...
      hours: Math.round(item.averageTime),
      cases: item.count,
    }))
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-3 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-amber-600 text-sm">Avg: {payload[0].value} hours</p>
  ...
  return (
    <ResponsiveContainer width="100%" height={300}>
      ...
      >
        ...
        <XAxis type="number" tick={{ fontSize: 12 }} label={{ value: 'Hours', position: 'insideBottom', offset: -10 }} />
        <YAxis 
          type="category" 
          dataKey="name" 
          tick={{ fontSize: 12 }}
